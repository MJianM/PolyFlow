...
def train():
    ...
    plot_simple_loss(loss_history, save_path='L_maze_train_loss.png')
    torch.save(model, "L_maze_final_model.pt")

    generated_traj = sample_discrete_delta(model, dataset, n_samples=2, steps=10)[-1] # (n_samples, seq_length*x_dim)

    true_traj = dataset.sample_traj_data(n_sample=2)

    plot_results(true_traj=true_traj, gene_traj=generated_traj, fig_name='L_maze_result.png')
